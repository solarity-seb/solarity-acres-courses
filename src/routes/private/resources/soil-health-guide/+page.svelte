<script lang="ts">
  ...
  async function checkEnrollment() {
    ...
    try {
      ...
      if (userResource) {
        ...
        goto('/resources/soil-health-guide');
        return;
      }
    } catch (err) {
      // User not enrolled
  ...
  async function updateUserProgress(newProgress: number) {
    ...
    if (newProgress > progress || isCompleted !== completed) {
      ...
      await updateResourceProgress(supabase, user.id, 'soil-health-guide', newProgress, isCompleted);
    }
  }
...
<SEO
  title="Complete Soil Health Guide - My Resources"
  description="Your personal copy of the Complete Soil Health Guide"
  url="/private/resources/soil-health-guide"
  noIndex={true}
/>
...
{:else if resource && enrolled}
  ...
  <PageTitle
    subheading="Your personal guide to building healthy, living soil"
    alignment="center"
    bg="bg2"
  >
    ...
    <div class="flex-grid">
      <div class="col-2">
        <nav class="table-of-contents sticky-nav">
          <h3>Contents</h3>
          <ul>
            <li><a href="#chapter-1">Understanding Soil Biology</a></li>
            <li><a href="#chapter-2">Soil Testing & Analysis</a></li>
            <li><a href="#chapter-3">Building Soil Structure</a></li>
            <li><a href="#chapter-4">Composting for Soil Health</a></li>
            <li><a href="#chapter-5">Natural Soil Amendments</a></li>
            <li><a href="#chapter-6">Seasonal Soil Care</a></li>
      ...
      <div class="col-8">
        <article class="content-article">
          {@html resource.content
            ...
            .replace(/^## Troubleshooting/gm, '<h2 id="troubleshooting">Troubleshooting')
            .replace(/^## Advanced Techniques/gm, '<h2 id="advanced">Advanced Techniques')
            .replace(/^### /gm, '<h3>')
            .replace(/^- /gm, '<li>')
            .replace(/^\d+\. /gm, '<li>')
